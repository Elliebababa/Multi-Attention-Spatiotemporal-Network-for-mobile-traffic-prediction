...
genneighbors = True
neighbnum = 15
data_file_path = '../data/processed/demo_internet_data.h5'#Nov_internet_data_t10_s3030_4070.h5'
test_len = 144 * 7 //3 - pre_len + 1
...
def makedataset(data, lookback = obs_len, prestep = pre_len, genneighbors = False, neighnum = 15):
    ...
    #   decoder_input_aux: [nodes, samples, pre_len , features2]
    ...
    return encoder_input_data, decoder_target_data, neighbor_values, neighbor_weight
...
def splitdata(data, testlen = test_len, shuffle = True):
    ...
    encoder_input_data, decoder_target_data, encoder_input_aux, neighbor_values, neighbor_weights = data
    train_encoder_input, train_decoder_target = encoder_input_data[:,:-testlen,], decoder_target_data[:,:-testlen,]
    test_encoder_input, test_decoder_target = encoder_input_data[:,-testlen:,], decoder_target_data[:,-testlen:,]
    ...
    return train_encoder_input_aux,test_encoder_input_aux,train_encoder_input, train_decoder_target, test_encoder_input, test_decoder_target,  train_decoder_input, \
...
if __name__ == '__main__':
    ...
    t1, t2, nei1, nei2 = makedataset(data, genneighbors = genneighbor)
    ...
    if genaux:
        f2 = h5.File('../data/processed/demo_call_data.h5','r')
        f3 = h5.File('../data/processed/demo_sms_data.h5','r')
        data2 = f2['data'].value
        data3 = f3['data'].value
    ...
    daux,daux2,d1,d2,d4,d5,d7,nv1,nv2,nw1,nw2 = splitdata([t1,t2,t4,t5,t6])
