...
def insample_validation(X, config, stepahead):
    ...
    mae_score = mae(testvalues, predictions)
    all_true_value.append(testvalues)
    all_predict_value.append(predictions)
    return nrmse_score,mae_score
...
def arima_test_all(dataX, cfg, stepahead,parallel = False, n_jobs = 32):
    ...
    print('all mae: %8f'%(mae(all_true_value,all_predict_value)))
    return scores
